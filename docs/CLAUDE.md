...
⚠⚠ **v1.6.0 とは逆で、今回は「タグを打ったが宣言管理下のノードに 1 台も届いていない」状態。**反映の受け皿は **pooza/chubo2#246**。⚠ **例外は dev27（ステージング）だけ** — 実機確認のため手で `8737a1c` に上げてあり、**そこでは v1.6.1 が実際に走っている**。⚠⚠ ただし `bin/chubo` を通していないので**宣言と実機がずれた状態**であり、**配備済みとは数えない**。
