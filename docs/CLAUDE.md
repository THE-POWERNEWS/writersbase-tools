...
- ✅ **`origin/develop` は 2026-09-19 に削除した。**マイルストーン完走までの修正を溜める想定だったが、⚠ **この規模の道具には合わない運用**で、実績としても一度も使われないまま `main` に対し **behind 112 / ahead 0** になっていた（固有コミット 0 件・tip の `8591f92` は `main` の祖先だったので、失われたものは無い）。⚠⚠ **復活させないこと。**
