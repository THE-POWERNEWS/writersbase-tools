...
### v1.6.1 は出荷済み（2026-09-18 タグ）—— ⚠ **まだ 1 台にも届いていない**

**最新タグは v1.6.1**（`8737a1c`／公開 2026-09-18）。`/package/version` も **1.6.1**。マイルストーン `1.6.1` の 3 件（#97 / #79 / #87）。

⚠⚠ **v1.6.0 とは逆で、今回は「タグを打ったが 1 台にも届いていない」状態。**反映の受け皿は **pooza/chubo2#246**。⚠ ステージングの **dev27 だけ、実機確認のため手で `8737a1c` に上げてある**（`bin/chubo` を通していないので宣言と実機がずれている）。
