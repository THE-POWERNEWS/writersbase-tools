...
### 1.7.1 は開発中（2026-09-25 時点・⚠ **まだタグを打っていない**）

`/package/version` は **1.7.1**（#131 でバンプ）。⚠⚠ **出荷済みの最新は v1.7.0 のまま。**
...
✅ **shallu / zugoga / gomander は 2026-09-25 に v1.7.0 へ上がった**（chubo2#246・`a5ffba3`。
`rake install` まで通り、`bundle install` を含む periodic スクリプトは 3 台とも 0 本）。
✅ **2026-09-25 に 3 台とも `git describe --tags` が `v1.7.0` と答えるのを実測した。**
⚠⚠ **vulcan は依然 v1.6.0**（Sentry の `release` で確認・09-23）。**vulcan と dev27 は chubo2#193 の後**の判断のまま。

⚠ **ステージング（dev27）は実機確認のため手で `cba2604` へ上げてある**（`bundle install` と
...
✅ **shallu / zugoga / gomander が 2026-09-18 に `8737a1c` で走り出した**（実測は 2026-09-19・chubo2#246）。⚠ **3 台とも 8 ソース全て成功・失敗 0**、`.rclonelink` の本数はローカルの symlink 数と完全一致、**破壊的変更（`postgresql_snapshot` の `dsn` 必須化）も通過して健在**。反映の受け皿 **pooza/chubo2#246 は vulcan / dev が残って open のまま**（FreeBSD 3 台は v1.7.0 まで済み）。
...
⚠ **当時の最新タグ**（`d3500df`／公開 2026-09-08）。⚠⚠ **いまの最新タグは v1.7.0・`/package/version` は 1.7.1**（上記）。⚠ **vulcan で走っているのはまだこの版**。v1.5.2（2026-04-12）から 48 コミットぶんで、マイルストーン `1.6.0` の 11 件・#37 Sentry 導入・misskey_emoji_sync 追加・#44 tootctl の login shell 経由をすべて含む。
...
### 反映後に見えた失敗（2026-09-25 時点・Sentry `writersbase-tools`）
...
- 🔴 **`-4` は 2026-09-25 時点で計 7 件。**09-23 に **vulcan で再発**（`/home/misskey/repos/misskey`・原因は同じクォータ）し、⚠⚠ **09-24 には初めて FreeBSD（zugoga・`release` 1.6.1）で出た。**ただし zugoga の件は**クォータではない**別の型 —— `/var/db/redis` の `dump.rdb` が**転送中に書き換わり**、3 回の試行とも `corrupted on transfer: md5 hashes differ` で落ちている（アップロードが 20 KB/s → 444 B/s まで落ちて 20 分かかり、その間に Redis の保存が走る）。⚠ **道具の欠陥ではなく「稼働中のファイルを直接 sync する」sources の問題**なので、直すのは chubo2 側（#194 の範囲）
