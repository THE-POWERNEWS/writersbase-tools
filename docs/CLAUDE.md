...
| #157 | ⏳ **#144 から切り出した後続**（tootctl が非ゼロ終了でも、出ている下書きは送ってから失敗として報告する） | ⏳ **pooza/mastodon#975 の仕様待ち**。⚠⚠ **#975 を zugoga へ入れる前に配ること**（逆順だとコピーに成功したぶんの告知が消える）。1.8.0 には入れない |
...
✅ **2026-09-26 に chubo2 側が配った**（受け皿 pooza/chubo2#254・記録は chubo2 `docs/infra-history.md`）。

| ノード | 版 | 備考 |
| --- | --- | --- |
| shallu / zugoga / gomander | ✅ v1.7.1 | v1.7.0 から。直後の hourly が成功し Kuma への push も 200（`0c54d3e`） |
...
- ⚠ **`announced: true` が何も投稿していない回にも出た**（`webhook.present?` をそのまま載せている）。**#144 に追記**（✅ 1.8.0 で「投稿できた件数」に変えた）
- ⚠ 直下の `/var/log/nginx/error.log.zst` は、**旧既定の `*.log` が生きた `error.log` を圧縮した跡**と見られる（#123 が塞いだ形の実例）

### v1.7.0（2026-09-20 タグ）

**当時の最新タグ**（`cba2604`／公開 2026-09-20）。マイルストーン `1.7.0` の 7 件。
✅ **shallu / zugoga / gomander は 2026-09-25 に v1.7.0 へ上がり**（chubo2#246・`a5ffba3`）、
**翌 09-26 に v1.7.1 へ進んだ**（上記）。⚠ dev24〜27・deas・pirazal / pirazis は 1.7.0 を飛ばして 1.7.1 へ入った。

⚠ 当時、dev27 は実機確認のため手で `cba2604` へ上げてあり、宣言と実機がずれていた（✅ 09-26 の v1.7.1 で解消）。
