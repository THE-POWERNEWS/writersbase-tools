...
| キー | 説明 | デフォルト |
| --- | --- | --- |
| dir | 対象ディレクトリ（再帰する） | /var/log/nginx |
| days | 経過日数 | 1 |
| patterns | 対象のファイル名（`File.fnmatch`） | `access_YYYYMMDD.log` の形（数字 8 桁） |

⚠⚠ **`patterns` を `*.log` のような広い形にしないこと。**圧縮は `zstd --rm` で元を消すので、
nginx が開いたままの `error.log` / `access.log` に当たると、nginx は削除済みのファイルへ書き続け、
**ログは静かに失われ、ディスクも空かない**（#123）。既定は rsyslog が日付で書くローテート済みの名前だけに当てる。
⚠ 配列なので、上書きすると既定は**置き換わる**（足し込みではない）。
...
⚠ **Webhook URLはそれ自体が資格情報**なので、`local.yaml`側に置いてください。⚠ URLはどのプロセスの引数にも載せていません（#127。以前は`--webhook <url>`としてtootctlへ渡していたので、実行中は`ps`から読めました）。ログでは`/logger/mask_url_paths`の`/webhook/`で伏せます。`origin`が未設定のときは「毎日静かに何もしない」状態を避けるため実行時にエラーにします。
